...
def extract_ids(resource_data, logger):
    ...
    try:
        ...
        logger.info(f"Extracted {len(id_list)} inventory count IDs")
        return id_list
...
async def fetch_resource(url, resource_name):
    """Main function to fetch all inventory count with pagination and save IDs."""
    ...
    logger.info(f"Total {len(all_items)} inventory count fetched")
...
async def list_inventory_count_id():
    """Entry point to fetch all inventory count id."""
    return await fetch_resource (INVENTORY_COUNT_ID_URL, "list_inventory_count_id")
