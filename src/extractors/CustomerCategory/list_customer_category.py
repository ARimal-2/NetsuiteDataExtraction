...
from urls import CUSTOMER_CATEGORY_LIST_URL
...
async def fetch_resource(url_template, resource_name, customer_ids):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async def fetch_customer_category(cust_id):
            items_for_customer = []
        ...
        # Launch all customer category fetches concurrently
        tasks = [fetch_customer_category(cust_id) for cust_id in customer_ids]
...
# -----------------------------
# Entry point for customer category list
# -----------------------------
async def list_customer_category(customer_ids):
    """Fetch all customers given the list of IDs"""
    return await fetch_resource(CUSTOMER_CATEGORY_LIST_URL, "list_customer_category", customer_ids)
