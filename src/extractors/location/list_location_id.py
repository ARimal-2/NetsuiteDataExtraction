...
def extract_ids(resource_data, logger):
    ...
    try:
        ...
        logger.info(f"Extracted {len(id_list)} location IDs")
        return id_list
...
async def fetch_resource(url, resource_name):
    """Main function to fetch all location with pagination and save IDs."""
    ...
    print(f"Final URL for fetching location: {next_url}")
    all_items = []
    ...
    logger.info(f"Total {len(all_items)} location fetched")
...
async def list_location_id():
    """Entry point to fetch all location."""
    return await fetch_resource(LOCATION_ID_URL, "list_location_id")
