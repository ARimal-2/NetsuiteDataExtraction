...
def extract_ids(resource_data, logger):
    ...
    try:
        ...
        logger.info(f"Extracted {len(id_list)} inventory transfer IDs")
        return id_list
...
async def fetch_resource(url, resource_name):
    """Main function to fetch all inventory number with pagination and save IDs."""
    ...
    print(f"Final URL for fetching inventory number: {next_url}")
    all_items = []
    ...
    logger.info(f"Total {len(all_items)} inventory number fetched")
...
async def list_inventory_transfer_id():
    """Entry point to fetch all inventory  id."""
    return await fetch_resource (INVENTORY_TRANSFER_ID_URL, "list_inventory_transfer_id")
