...
# -------------------------------------------------------
async def fetch_resource(url, resource_name, transfer_ids):
    ...
    for transfer_id in transfer_ids:
        next_url = url.replace("{id}", str(transfer_id))

        while next_url:
            try:
                data = await fetch_resource_data(next_url, logger)
            except Exception:
                logger.warning(f"Skipping inventory {transfer_id} due to fetch error.")
                break
            ...
            logger.info(f"Fetched {len(items)} items for inventory {transfer_id}")
