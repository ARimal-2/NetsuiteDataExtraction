...
# -----------------------------
async def fetch_resource(url_template, resource_name, number_id):
    """
    Fetch multiple inventory_numbers concurrently with pagination.
    """
    ...
    if not number_id:
        logger.warning("inventory_number ID list is empty")
        return resource_name, [], number_id
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async def fetch_inventory_number(n_id):
            items_for_inventory_number = []
            next_url = url_template.replace("{id}", str(n_id))
            ...
            while next_url:
                ...
                items_for_inventory_number.extend(items)
                logger.info(f"Fetched {len(items)} items for assembly item ID {n_id} from {next_url}")
                # Pagination
                next_links = [link["href"] for link in data.get("links", []) if link.get("rel") == "next"]
                next_url = next_links[0] if next_links else None


            return items_for_inventory_number

        # Launch all inventory_number fetches concurrently
        tasks = [fetch_inventory_number(n_id) for n_id in number_id]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        for r in results:
            if isinstance(r, Exception):
                logger.error(f"Error in inventory_number task: {r}")
            else:
                all_items.extend(r)
    ...
    return resource_name, all_items, number_id


# -----------------------------
# Entry point for inventory_number list
# -----------------------------
async def list_inventory_details(number_id):
    """Fetch all inventory_numbers given the list of IDs"""
    return await fetch_resource(INVENTORY_NUMBER_URL, "list_inventory_details", number_id)
