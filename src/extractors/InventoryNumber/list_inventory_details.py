...
# -------------------------------------------------------
async def fetch_resource(url, resource_name, numbers):
    ...
    if not numbers:
        logger.warning("inventory ID list is empty")
        return resource_name, [], numbers
    ...
    for number in numbers:
        next_url = url.replace("{id}", str(number))

        while next_url:
            try:
                data = await fetch_resource_data(next_url, logger)
            except Exception:
                logger.warning(f"Skipping inventory {number} due to fetch error.")
                break
            ...
            logger.info(f"Fetched {len(items)} items for inventory {number}")
    ...
    return resource_name, all_items, numbers


# -------------------------------------------------------
# Entry for inventory id list
# -------------------------------------------------------
async def list_inventory_details(number):
    """Entry point to fetch all inventory ids using provided ID list."""
    return await fetch_resource( INVENTORY_DETAILS_URL, "list_inventory_details", number)
