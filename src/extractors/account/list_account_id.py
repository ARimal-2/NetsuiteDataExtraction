...
def extract_ids(resource_data, logger):
    ...
    try:
        ...
        logger.info(f"Extracted {len(id_list)} account IDs")
        return id_list
...
async def fetch_resource(url, resource_name):
    """Main function to fetch all account with pagination and save IDs."""
    ...
    print(f"Final URL for fetching account: {next_url}")
    all_items = []
    ...
    logger.info(f"Total {len(all_items)} accounts fetched")
...
async def list_account_id():
    """Entry point to fetch all account."""
    return await fetch_resource(ACCOUNT_ID_URL, "list_account_id")
