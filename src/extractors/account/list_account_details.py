...
# -----------------------------
async def fetch_resource(url_template, resource_name, account_ids):
    """
    Fetch multiple accounts concurrently with pagination.
    """
    ...
    if not account_ids:
        logger.warning("account ID list is empty")
        return resource_name, [], account_ids
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async def fetch_account(acc_id):
            items_for_account = []
            next_url = url_template.replace("{id}", str(acc_id))
            ...
            while next_url:
                ...
                items_for_account.extend(items)
                logger.info(f"Fetched {len(items)} items for account ID {acc_id} from {next_url}")

                # Pagination
                next_links = [link["href"] for link in data.get("links", []) if link.get("rel") == "next"]
                next_url = next_links[0] if next_links else None
                logger.info(f"Fetched {len(items)} items for assembly item ID {acc_id} from {next_url}")

            return items_for_account

        # Launch all account fetches concurrently
        tasks = [fetch_account(acc_id) for acc_id in account_ids]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        for r in results:
            if isinstance(r, Exception):
                logger.error(f"Error in account task: {r}")
            else:
                all_items.extend(r)
    ...
    return resource_name, all_items, account_ids


# -----------------------------
# Entry point for account list
# -----------------------------
async def list_account_details(accouunt_details):
    """Fetch all account given the list of IDs"""
    return await fetch_resource(ACCOUNT_DETAILS_URL, "list_account_details", accouunt_details)
