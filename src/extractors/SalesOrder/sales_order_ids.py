...
def extract_ids(resource_data, logger):
    ...
    try:
        ...
        logger.info(f"Extracted {len(id_list)} sales order IDs")
        return id_list
...
async def list_sales_order_ids():
    """Entry point to fetch all sale order."""
    return await fetch_resource(SALES_ORDER_ID_URL, "list_sales_order_ids")
