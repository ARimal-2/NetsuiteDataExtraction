...
def extract_ids(resource_data, logger):
    ...
    try:
        ...
        logger.info(f"Extracted {len(id_list)} purchase order IDs")
        return id_list
...
async def fetch_resource(url, resource_name):
    """Main function to fetch all purchase order with pagination and save IDs."""
    ...
    print(f"Final URL for fetching purchase order: {next_url}")
    all_items = []
    ...
    logger.info(f"Total {len(all_items)} purchase order fetched")
...
async def list_purchase_order_id():
    """Entry point to fetch all purchase order."""
    return await fetch_resource(PURHCHASE_ORDER_ID_URL, "list_purchase_order_id")
