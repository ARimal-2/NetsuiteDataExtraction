...
# -----------------------------
async def fetch_resource(url_template, resource_name, PoId):
    """
    Fetch multiple purchase orders concurrently with pagination.
    """
    ...
    if not PoId:
        logger.warning("purchase order ID list is empty")
        return resource_name, [], PoId
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async def fetch_purchase_order(p_id):
            items_for_purchase_order = []
            next_url = url_template.replace("{id}", str(p_id))
            ...
            while next_url:
                ...
                items_for_purchase_order.extend(items)
                logger.info(f"Fetched {len(items)} items for purchase order ID {p_id} from {next_url}")
                # Pagination
                next_links = [link["href"] for link in data.get("links", []) if link.get("rel") == "next"]
                next_url = next_links[0] if next_links else None

            return items_for_purchase_order

        # Launch all purchase order fetches concurrently
        tasks = [fetch_purchase_order(p_id) for p_id in PoId]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        for r in results:
            if isinstance(r, Exception):
                logger.error(f"Error in purchase order task: {r}")
            else:
                all_items.extend(r)
...
# -----------------------------
# Entry point for purchase order list
# -----------------------------
async def list_purchase_order(PoId):
    """Fetch all purchase orders given the list of IDs"""
    return await fetch_resource(PURCHASE_ORDER_DETAILS_URL, "list_purchase_order", PoId)
