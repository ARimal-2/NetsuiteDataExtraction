...
from urls import CUSTOMER_SUBSIDIARY_RELATIONSHIP_ID_URL
import urllib.parse
...
async def fetch_resource(url, resource_name):
    """Main function to fetch all customers with pagination and save IDs."""
    ...
    print(f"Final URL for fetching customers: {next_url}")
    all_items = []
    ...
    logger.info(f"Total {len(all_items)} customers fetched")
...
async def list_customer_subsidiary_relationship_id():
    """Entry point to fetch all customers."""
    return await fetch_resource (CUSTOMER_SUBSIDIARY_RELATIONSHIP_ID_URL, "list_customer_subsidiary_relationship_id")
