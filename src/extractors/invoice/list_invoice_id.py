...
def extract_ids(resource_data, logger):
    ...
    try:
        ...
        logger.info(f"Extracted {len(id_list)} invoice IDs")
        return id_list
...
async def fetch_resource(url, resource_name):
    """Main function to fetch all invoice with pagination and save IDs."""
    ...
    print(f"Final URL for fetching invoice: {next_url}")
    all_items = []
    ...
    logger.info(f"Total {len(all_items)} invoice fetched")
...
async def list_invoice_id():
    """Entry point to fetch all invoice."""
    return await fetch_resource(INVOICE_ID_URL, "list_invoice_id")
