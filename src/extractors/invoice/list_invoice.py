...
async def fetch_resource(url_template, resource_name, invoice_ids):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async def fetch_invoice(inv_id):
            items_for_invoice = []
            next_url = url_template.replace("{id}", str(inv_id))
            ...
            while next_url:
                ...
                logger.info(f"Fetched {len(items)} items for invoice ID {inv_id} from {next_url}")
        ...
        tasks = [fetch_invoice(inv_id) for inv_id in invoice_ids]
