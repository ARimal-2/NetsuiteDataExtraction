...
# -----------------------------
async def fetch_resource(url_template, resource_name, vendorCategory_ids):
    """
    Fetch multiple vendorCategorys concurrently with pagination.
    """
    ...
    if not vendorCategory_ids:
        logger.warning("vendorCategory ID list is empty")
        return resource_name, [], vendorCategory_ids
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async def fetch_vendorCategory(vc_id):
            items_for_vendorCategory = []
            next_url = url_template.replace("{id}", str(vc_id))
            ...
            while next_url:
                ...
                items_for_vendorCategory.extend(items)
                logger.info(f"Fetched {len(items)} items for vendorCategory ID {vc_id} from {next_url}")

                # Pagination
                next_links = [link["href"] for link in data.get("links", []) if link.get("rel") == "next"]
                next_url = next_links[0] if next_links else None

            return items_for_vendorCategory

        # Launch all vendorCategory fetches concurrently
        tasks = [fetch_vendorCategory(vc_id) for vc_id in vendorCategory_ids]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        for r in results:
            if isinstance(r, Exception):
                logger.error(f"Error in vendorCategory task: {r}")
            else:
                all_items.extend(r)
    ...
    return resource_name, all_items, vendorCategory_ids


# -----------------------------
# Entry point for vendorCategory list
# -----------------------------
async def list_vendorCategory_details(vendorCategory_ids):
    """Fetch all vendorCategory given the list of IDs"""
    return await fetch_resource(VENDOR_DETAILS_URL, "list_vendorCategory_details", vendorCategory_ids)
