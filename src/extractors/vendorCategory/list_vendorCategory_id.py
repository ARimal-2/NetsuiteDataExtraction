...
def extract_ids(resource_data, logger):
    ...
    try:
        ...
        logger.info(f"Extracted {len(id_list)} vendorCategory IDs")
        return id_list
...
async def fetch_resource(url, resource_name):
    """Main function to fetch all vendorCategory with pagination and save IDs."""
    ...
    print(f"Final URL for fetching vendorCategory: {next_url}")
    all_items = []
...
async def list_vendorCategory_id():
    ...
    return await fetch_resource(VENDOR_CATEGORY_ID_URL, "list_vendorCategory_id")
