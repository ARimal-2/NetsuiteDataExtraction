...
def extract_ids(resource_data, logger):
    ...
    try:
        ...
        logger.info(f"Extracted {len(id_list)} subsidiary IDs")
        return id_list
...
async def fetch_resource(url, resource_name):
    """Main function to fetch all subsidiary with pagination and save IDs."""
    ...
    print(f"Final URL for fetching subsidiary: {next_url}")
    all_items = []
    ...
    logger.info(f"Total {len(all_items)} subsidiary fetched")
...
async def list_subsidiary_id():
    """Entry point to fetch all subsidiary."""
    return await fetch_resource(SUBSIDIARY_ID_URL, "list_subsidiary_id")
