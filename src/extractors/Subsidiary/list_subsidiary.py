...
# -----------------------------
async def fetch_resource(url_template, resource_name, subsidiary_ids):
    """
    Fetch multiple subsidiarys concurrently with pagination.
    """
    ...
    if not subsidiary_ids:
        logger.warning("subsidiary ID list is empty")
        return resource_name, [], subsidiary_ids
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async def fetch_subsidiary(subsi_id):
            items_for_subsidiary = []
            next_url = url_template.replace("{id}", str(subsi_id))
            ...
            while next_url:
                ...
                items_for_subsidiary.extend(items)
                logger.info(f"Fetched {len(items)} items for subsidiary ID {subsi_id} from {next_url}")

                # Pagination
                next_links = [link["href"] for link in data.get("links", []) if link.get("rel") == "next"]
                next_url = next_links[0] if next_links else None

            return items_for_subsidiary

        # Launch all subsidiary fetches concurrently
        tasks = [fetch_subsidiary(subsi_id) for subsi_id in subsidiary_ids]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        for r in results:
            if isinstance(r, Exception):
                logger.error(f"Error in subsidiary task: {r}")
            else:
                all_items.extend(r)
    ...
    return resource_name, all_items, subsidiary_ids


# -----------------------------
# Entry point for subsidiary list
# -----------------------------
async def list_subsidiary_details(subsidiary_ids):
    """Fetch all subsidiary given the list of IDs"""
    return await fetch_resource(SUBSIDIARY_DETAILS_URL, "list_subsidiary_details", subsidiary_ids)
