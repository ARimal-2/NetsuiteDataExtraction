...
def extract_ids(resource_data, logger):
    ...
    try:
        ...
        logger.info(f"Extracted {len(id_list)} itemReceipt IDs")
        return id_list
...
async def fetch_resource(url, resource_name):
    """Main function to fetch all itemReceipt with pagination and save IDs."""
    ...
    print(f"Final URL for fetching itemReceipt: {next_url}")
    all_items = []
    ...
    logger.info(f"Total {len(all_items)} itemReceipt fetched")
...
async def list_itemReceipt_id():
    """Entry point to fetch all itemReceipt."""
    return await fetch_resource(ITEM_RECEIPT_ID_URL, "list_itemReceipt_id")
