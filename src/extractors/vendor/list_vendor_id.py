...
def extract_ids(resource_data, logger):
    ...
    try:
        ...
        logger.info(f"Extracted {len(id_list)} vendor IDs")
        return id_list
...
async def fetch_resource(url, resource_name):
    """Main function to fetch all vendor with pagination and save IDs."""
    ...
    print(f"Final URL for fetching vendor: {next_url}")
    all_items = []
    ...
    logger.info(f"Total {len(all_items)} vendor fetched")
...
async def list_vendor_id():
    """Entry point to fetch all vendor."""
    return await fetch_resource(VENDOR_ID_URL, "list_vendor_id")
