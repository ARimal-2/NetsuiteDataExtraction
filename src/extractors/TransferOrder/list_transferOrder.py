...
# -----------------------------
async def fetch_resource(url_template, resource_name, trans_order_ids):
    """
    Fetch multiple transferOrders concurrently with pagination.
    """
    ...
    if not trans_order_ids:
        logger.warning("transferOrder ID list is empty")
        return resource_name, [], trans_order_ids
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async def fetch_transferOrder(tOrder_id):
            items_for_transferOrder = []
            next_url = url_template.replace("{id}", str(tOrder_id))
            ...
            while next_url:
                ...
                items_for_transferOrder.extend(items)
                logger.info(f"Fetched {len(items)} items for transferOrder ID {tOrder_id} from {next_url}")

                # Pagination
                next_links = [link["href"] for link in data.get("links", []) if link.get("rel") == "next"]
                next_url = next_links[0] if next_links else None

            return items_for_transferOrder

        # Launch all transferOrder fetches concurrently
        tasks = [fetch_transferOrder(tOrder_id) for tOrder_id in trans_order_ids]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        for r in results:
            if isinstance(r, Exception):
                logger.error(f"Error in transferOrder task: {r}")
            else:
                all_items.extend(r)
    ...
    return resource_name, all_items, trans_order_ids


# -----------------------------
# Entry point for transferOrder list
# -----------------------------
async def list_transferOrder_details(trans_order_ids):
    """Fetch all transferOrder given the list of IDs"""
    return await fetch_resource(TRANSFER_ORDER_DETAILS_URL, "list_transferOrder_details", trans_order_ids)
