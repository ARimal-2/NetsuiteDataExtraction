...
def extract_ids(resource_data, logger):
    ...
    try:
        ...
        logger.info(f"Extracted {len(id_list)} transferOrder IDs")
        return id_list
...
async def fetch_resource(url, resource_name):
    """Main function to fetch all transferOrder with pagination and save IDs."""
    ...
    print(f"Final URL for fetching transferOrder: {next_url}")
    all_items = []
    ...
    logger.info(f"Total {len(all_items)} transferOrder fetched")
...
async def list_transferOrder_id():
    """Entry point to fetch all transferOrder."""
    return await fetch_resource(TRANSFER_ORDER_ID_URL, "list_transferOrder_id")
