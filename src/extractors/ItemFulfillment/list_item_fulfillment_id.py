...
def extract_ids(resource_data, logger):
    ...
    try:
        ...
        logger.info(f"Extracted {len(id_list)} item fulfillment IDs")
        return id_list
...
async def fetch_resource(url, resource_name):
    """Main function to fetch all item fulfillment with pagination and save IDs."""
    ...
    print(f"Final URL for fetching item fulfillment: {next_url}")
    all_items = []
    ...
    logger.info(f"Total {len(all_items)} item fulfillment fetched")
...
async def list_item_fulfillment_id():
    """Entry point to fetch all item fulfillment."""
    return await fetch_resource(ITEM_FULFILLMENT_ID_URL, "list_item_fulfillment_id")
