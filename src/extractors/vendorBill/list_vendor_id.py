...
def extract_ids(resource_data, logger):
    ...
    try:
        ...
        logger.info(f"Extracted {len(id_list)} vendorBill IDs")
        return id_list
...
async def fetch_resource(url, resource_name):
    """Main function to fetch all vendorBill with pagination and save IDs."""
    ...
    print(f"Final URL for fetching vendorBill: {next_url}")
    all_items = []
    ...
    logger.info(f"Total {len(all_items)} vendorBill fetched")
...
async def list_vendorBill_id():
    ...
    return await fetch_resource(VENDOR_BILL_ID_URL, "list_vendorBill_id")
