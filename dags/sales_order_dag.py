...
with DAG(
    dag_id="netsuite_sales_order_dag",
    ...
    # inventory items
    # ---------------------------
    def fetch_sales_orders_ids(**context):
        try:
            ...
            if id_list:
                r.rpush("sales_order_id_queue", *id_list)
                print(f"Pushed {len(id_list)} sales order IDs to Redis queue")
            if resource_data:
                asyncio.run(safe_upload(resource_data, resource_name))
            else:
                logger.warning("No IDs returned from list_sales_order_id")
        except Exception as e:
            logger.error(f"fetch_sales_orders_ids failed: {e}", exc_info=True)
            raise

    def fetch_sales_orders_details(**context):
        try:
            all_ids = []
            while True:
                invent_id = r.lpop("sales_order_id_queue")
                if invent_id is None:
                    break
                all_ids.append(invent_id.decode('utf-8'))

            resource_name, all_items = asyncio.run(list_sales_order(all_ids))
            if all_items:
                asyncio.run(safe_upload(all_items, resource_name))
            else:
                logger.warning("list_sales_orders returned no data")
        except Exception as e:
            logger.error(f"fetch_inventory items_details failed: {e}", exc_info=True)
            raise

    list_sales_orders_ids_task = PythonOperator(
        task_id="list_sales_orders_ids",
        python_callable=fetch_sales_orders_ids 
    )

    list_sales_orders_details_task = PythonOperator(
        task_id="list_sales_orders_details",      # FIXED
        python_callable=fetch_sales_orders_details
    )

    list_sales_orders_ids_task >> list_sales_orders_details_task
